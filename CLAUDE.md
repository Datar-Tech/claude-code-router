...
-   **Dependencies**: The project is built with `esbuild`. It has a key local dependency `@Datar-Tech/llms`, which probably contains the core logic for interacting with different LLM APIs.
-   `@Datar-Tech/llms` is implemented based on `fastify` and exposes `fastify`'s hook and middleware interfaces, allowing direct use of `server.addHook`.